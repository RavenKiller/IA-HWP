...
def angle_distance_feature(headings, distances, dim=64, device=None):
    heading_enc = torch.zeros(len(headings), dim, dtype=torch.float32)
    distance_enc = torch.zeros(len(distances), dim, dtype=torch.float32)
    for i, head in enumerate(headings):
        heading_enc[i] = torch.tensor([math.sin(head), math.cos(head)] * (dim // 2))
        dist = distances[i]
        distance_enc[i] = torch.tensor([dist] * dim)
    hd_code = torch.cat((heading_enc, distance_enc), dim=1)
    return hd_code.to(device)
