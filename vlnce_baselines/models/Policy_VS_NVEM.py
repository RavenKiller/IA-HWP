...
class NvemNet(Net):
    ...
    def __init__(
        ...
        self.value_action = nn.Sequential(
            nn.Linear(
                model_config.VISUAL_DIM.directional, model_config.VISUAL_DIM.vis_hidden
            ),
            nn.Tanh(),
        )
    ...
    def forward(
        ...
        if mode == "language":
            ...
            for i, k in enumerate(depth_sensor_ids):
                for bi in range(
                    observations[k].size(0)
                ):  # observations是个字典包数组的, bi是batch idx
            ...
            state_in = torch.cat(
                [vis_prev_state, prev_actions], dim=1
            )  # [B,576] h_{t-1}
            ...
            fusion_weight = torch.cat(
                [self.fuse_vis(text_state), self.fuse_act(action_state)], dim=-1
            )
            ...
            vis_text_feats, _ = self.text_vis_attn(text_state, vis_in, cand_mask)

            # action value [2,13,512]
            action_in = self.value_action(cand_direction)
            # 从12个dir中注意一个
            action_feat, _ = self.action_vis_attn(action_state, action_in, cand_mask)
            ...
            logits = torch.cat(
                [logits_vis.unsqueeze(2), logits_act.unsqueeze(2)], dim=-1
            )
